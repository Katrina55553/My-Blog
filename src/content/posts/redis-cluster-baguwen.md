...
```mermaid
flowchart TD
    A[Redis Cluster 多个节点] --> B[采用虚拟槽分区<br>16384个槽]
    B --> C{计算数据归属}
    C --> D["CRC16(key) % 16384"]
    D --> E[数据存入对应槽所在节点]
    
    E --> F[节点间通过Gossip协议<br>交换元信息]
    F --> G[每个节点感知集群全貌]
    
    G --> H{客户端请求}
    H -- key在本节点 --> I[直接返回数据]
    H -- key不在本节点且槽已迁移 --> J[返回 MOVED 错误<br>客户端更新映射]
    H -- 槽正在迁移中 --> K[返回 ASK 错误<br>客户端临时重定向]
    
    subgraph 高可用保障
        L[每个主节点有从节点<br>通过主从复制同步]
        M[主节点宕机后<br>从节点自动提升]
    end
```
